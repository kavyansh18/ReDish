...
const ReDish = () => {
  ...
  return (
    <div className="p-4 bg-redish-bg min-h-screen">
      <h1 className="lg:text-7xl text-5xl pt-5 font-bold text-redish-accent unlock-regular flex justify-center items-center mb-1">ReDish</h1>
      <p className="mb-3 text-redish-text lg:text-2xl text-sm flex justify-center items-center">Discover Indian recipes with your leftover ingredients!</p>
      <div className="shadow-2xl h-[42rem] lg:w-[90%] w-[100%] mx-auto bg-redish-card rounded-[36px] lg:scale-100 scale-[.9]">
        <button 
          className="smky-btn3 relative hover:text-redish-text py-2 px-6 after:absolute after:h-1 after:hover:h-[200%] transition-all duration-500 hover:transition-all hover:duration-500 after:transition-all after:duration-500 after:hover:transition-all after:hover:duration-500 overflow-hidden z-20 after:z-[-20] after:bg-redish-accent after:rounded-t-full after:w-full after:bottom-0 after:left-0 text-gray-600 font-semibold cursor-pointer"
          onClick={() => {
            setCounter(0);
            setChat([]);
          }}
        ...
        <div className="h-[35.6rem] overflow-y-auto p-4" ref={chatContainerRef}>
          {chat.length === 0 && (
            <h1 className="fixed lg:text-[40px] text-2xl font-semibold text-redish-text left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            >
              ...
              <div
                className={`max-w-[70%] px-5 py-3 rounded-[36px] ${message.sender === 'user'
                    ? 'bg-redish-accent text-white self-end'
                    : 'bg-redish-secondary text-gray-900'
                  }`}
              >
        ...
        <div className="flex justify-end items-center mr-5">
          <input
            className="input h-[46px] mt-2 text-[14px] text-white/60 w-[450px] bg-redish-bg text-redish-text px-3 py-1 rounded-[36px] border border-white/10 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 focus:ring-offset-redish-accent transition-all duration-150 ease-in-out ml-4"
            name="text"
            type="text"
          ...
          <span>
            <img
              className="lg:w-12 w-[70px] mt-2 cursor-pointer hover:bg-redish-bg rounded-[36px] ml-1 p-1"
              src={send}
              alt="Send"
